...
function ConfigurableAssetsTable({assets,settings,reportByAsset,query,setQuery,status,setStatus,type,setType,location,setLocation,onImport,onExport,onCreate,onOpenAsset}){
  ...
  return <div className="nv-assets-card card">
    ...
    {activeFilters.length>0&&<div style={{display:'flex',gap:'8px',flexWrap:'wrap',padding:'0 12px 12px'}}>{activeFilters.map(key=>{const col=columns.find(c=>c.key===key);return <div key={key} style={{display:'flex',alignItems:'center',gap:'6px'}}><input style={{minWidth:'160px'}} placeholder={\`Filter by \${col?.label||key}…\`} value={filterValues[key]||''} onChange={e=>setFilterValues({...filterValues,[key]:e.target.value})}/><button className="secondary" onClick={()=>removeFilter(key)}>×</button></div>;})}</div>}
