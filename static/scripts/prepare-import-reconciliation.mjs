...
const oldImport = "  async function importAssets(rows){const result=await invoke('bulkImportAssets',{assets:rows});setMessage(`Import complete: ${result.imported} created${result.failed?.length?`, ${result.failed.length} failed`:''}${result.deviceTypesAdded?.length?`. Added device types: ${result.deviceTypesAdded.join(', ')}`:''}`);await load();}";
...
else if(!main.includes("invoke('reconcileAssetImport'")) throw new Error('Could not locate Asset Manager import handler.');
