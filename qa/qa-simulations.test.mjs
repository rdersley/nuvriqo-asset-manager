...
test('large Jira discovery is resumable and bounded per Forge invocation', () => { assert.match(backend,/SYNC_PROGRESS_KEY/); assert.match(backend,/SYNC_JIRA_PAGE_SIZE\s*=\s*100/); assert.match(backend,/searchIssuePageWithConfiguredAssetField/); assert.match(backend,/nextPageToken:progress\?\.nextPageToken/); assert.match(backend,/else await kvs\.set\(SYNC_PROGRESS_KEY,progress\)/); assert.match(frontend,/while\(sync&&!sync\.complete&&guard<50\)/); assert.match(frontend,/Syncing Jira devices…/); });
