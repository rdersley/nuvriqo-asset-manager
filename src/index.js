...
import { publishPortalPlusSnapshotsForMappings } from './portal-plus-publisher.js';
...
async function refreshPortalPlusSnapshots(){
  try{
    const {field,issues,settings}=await searchIssuesWithConfiguredAssetField();
    if(!field)return{ok:true,published:0,skipped:true,reason:'asset-field-not-configured'};
    const organisationField=(await getJiraCustomFields()).find((f)=>normaliseName(f.name)==='organizations'||normaliseName(f.name)==='organisations'||normaliseName(f.customType).includes('organization'));
    if(!organisationField)return{ok:true,published:0,skipped:true,reason:'organisation-field-not-found'};
    const assets=await queryAllByPrefix(ASSET_PREFIX);
    return await publishPortalPlusSnapshotsForMappings({issues,assets,deviceFieldId:field.id,organisationFieldId:organisationField.id});
  }catch(error){console.warn('Portal+ asset snapshot refresh failed',error?.message||error);return{ok:false,published:0,error:String(error?.message||error)};}
}
...
async function resolveJiraAssetField(){const settings=await getSettingsValue();const fields=await getJiraCustomFields();if(settings.jiraAssetField?.id){const selected=fields.find(f=>f.id===settings.jiraAssetField.id);if(selected)return selected;}const preferredNames=['device id','asset id','asset name','device name','asset','device'];return fields.find(f=>preferredNames.includes(normaliseName(f.name)))||null;}
async function searchIssuesWithConfiguredAssetField(){const settings=await getSettingsValue();const field=await resolveJiraAssetField();if(!field)return{field:null,issues:[],settings};const numericId=String(field.id).replace('customfield_','');const jql=`cf[${numericId}] is not EMPTY ORDER BY created DESC`;const fields=[field.id,settings.jiraRelatedAssetField?.id,settings.jiraLocationField?.id,settings.jiraTypeField?.id,settings.jiraCrewCodeField?.id,'summary','status','issuetype','priority','assignee','created','resolutiondate','resolution'].filter(Boolean);const issues=[];let nextPageToken;do{const body={jql,fields,maxResults:100};if(nextPageToken)body.nextPageToken=nextPageToken;const response=await api.asUser().requestJira(route`/rest/api/3/search/jql`,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json'},body:JSON.stringify(body)});if(!response.ok)throw new Error(`Jira asset-field lookup failed with status ${response.status}.`);const data=await response.json();issues.push(...safeArray(data.issues));nextPageToken=data.nextPageToken||null;}while(nextPageToken);return{field,issues,settings};}
...
async function syncAssetsFromJira({restart=false}={}){
  ...
  if(restart||!progress||progress.fieldId!==field.id||progress.total!==identifiers.length){const reconciled=await reconcileJiraAssets();progress={fieldId:field.id,cursor:0,total:identifiers.length,created:0,matched:0,reconciled,startedAt:now()};await kvs.set(SYNC_PROGRESS_KEY,progress);await kvs.delete(SYNC_KEY);}
  const assets=await queryAllByPrefix(ASSET_PREFIX);const byIdentifier=new Map(),byLegacyName=new Map();for(const asset of assets){if(asset.jiraIdentifier)byIdentifier.set(normaliseName(asset.jiraIdentifier),asset);if(asset.name)byLegacyName.set(normaliseName(asset.name),asset);}
  const start=progress.cursor;const slice=identifiers.slice(start,start+SYNC_BATCH_SIZE);let batchCreated=0,batchMatched=0;
  for(const identifier of slice){
    const normalized=normaliseName(identifier);let asset=byIdentifier.get(normalized)||byLegacyName.get(normalized)||null;const deterministicId=makeJiraAssetId(field.id,identifier);if(!asset)asset=await kvs.get(`${ASSET_PREFIX}${deterministicId}`);
    const location=latestFieldValueForIdentifier(issues,field.id,identifier,settings.jiraLocationField?.id);const type=latestFieldValueForIdentifier(issues,field.id,identifier,settings.jiraTypeField?.id);const crewCode=latestFieldValueForIdentifier(issues,field.id,identifier,settings.jiraCrewCodeField?.id);const crewPerson=crewCode?mappedCrewPerson(settings,crewCode):null;const holderName=crewCode?(crewPerson?.displayName||crewCode):'';const holderAccountId=crewPerson?.accountId||'';const base={jiraIdentifier:identifier,jiraIdentifierFieldId:field.id,jiraIdentifierFieldName:field.name};
    if(asset){asset=await saveOneAsset({...asset,...base,...(location?{location}:{}),...(type?{type}:{}),...(crewCode?{crewCode,assigneeName:holderName,assigneeAccountId:holderAccountId}:{})},'jira-sync');batchMatched+=1;}else{asset=await saveOneAsset({id:deterministicId,name:identifier,...base,type:type||'Other',status:'In Use',location:location||'',crewCode:crewCode||'',assigneeAccountId:holderAccountId,assigneeName:holderName,notes:`Discovered automatically from Jira field “${field.name}”.`},'jira-sync');batchCreated+=1;}
    byIdentifier.set(normalized,asset);byLegacyName.set(normaliseName(asset.name),asset);
  }
  const processed=Math.min(start+slice.length,identifiers.length);progress={...progress,cursor:processed,created:progress.created+batchCreated,matched:progress.matched+batchMatched};const complete=processed>=identifiers.length;const result={timestamp:now(),field,issuesScanned:issues.length,discovered:identifiers.length,processed,created:progress.created,matched:progress.matched,ignored,reconciled:progress.reconciled,complete};if(complete){await kvs.set(SYNC_KEY,result);await kvs.delete(SYNC_PROGRESS_KEY);}else await kvs.set(SYNC_PROGRESS_KEY,progress);return result;
}
async function searchAssetTickets(assetId,legacyKeys=[]){
  ...
  if(legacyKeys.length){const missingKeys=legacyKeys.filter(k=>!matched.has(k));if(missingKeys.length){const response=await api.asUser().requestJira(route`/rest/api/3/search/jql`,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json'},body:JSON.stringify({jql:`key in (${missingKeys.join(',')}) ORDER BY created DESC`,fields:['summary','status','issuetype','priority','assignee','created','resolutiondate','resolution'],maxResults:100})});if(response.ok){const data=await response.json();for(const issue of safeArray(data.issues))matched.set(issue.key,ticketFields(issue,'linked'));}}}
  return [...matched.values()].sort((a,b)=>String(b.created||'').localeCompare(String(a.created||'')));
}
...
resolver.define('syncAssetsFromJira',async({payload})=>{const result=await syncAssetsFromJira({restart:Boolean(payload?.restart)});if(result?.complete)await refreshPortalPlusSnapshots();return result;});
...
resolver.define('saveSettings',async({payload})=>{const incoming=payload?.settings||{};const normaliseField=(f)=>f?.id?{id:clean(f.id),name:clean(f.name||f.id)}:null;const settings={assetTypes:safeArray(incoming.assetTypes).map(clean).filter(Boolean),statuses:safeArray(incoming.statuses).map(clean).filter(Boolean),locations:safeArray(incoming.locations).map(clean).filter(Boolean),customFields:safeArray(incoming.customFields).map(f=>({key:clean(f.key),label:clean(f.label),type:clean(f.type||'text')})).filter(f=>f.key&&f.label),jiraAssetField:normaliseField(incoming.jiraAssetField),jiraRelatedAssetField:normaliseField(incoming.jiraRelatedAssetField),jiraLocationField:normaliseField(incoming.jiraLocationField),jiraTypeField:normaliseField(incoming.jiraTypeField),jiraCrewCodeField:normaliseField(incoming.jiraCrewCodeField),crewMappings:safeArray(incoming.crewMappings).map(m=>({crewCode:clean(m.crewCode),accountId:clean(m.accountId),displayName:clean(m.displayName)})).filter(m=>m.crewCode&&(m.displayName||m.accountId)),jiraDiscoveryEnabled:incoming.jiraDiscoveryEnabled!==false};if(!settings.assetTypes.length)settings.assetTypes=DEFAULT_SETTINGS.assetTypes;if(!settings.statuses.length)settings.statuses=DEFAULT_SETTINGS.statuses;await kvs.set(SETTINGS_KEY,settings);await kvs.delete(SYNC_KEY);await kvs.delete(SYNC_PROGRESS_KEY);await refreshPortalPlusSnapshots();return settings;});
...
resolver.define('getIssueContext',async({context})=>{const issueKey=context?.extension?.issue?.key;if(!issueKey)return{issueKey:null,linkedAsset:null};const link=await kvs.get(`${LINK_PREFIX}${issueKey}`);const linkedAsset=link?.assetId?await kvs.get(`${ASSET_PREFIX}${link.assetId}`):null;return{issueKey,linkedAsset};});
resolver.define('linkAssetToIssue',async({payload,context})=>{const issueKey=payload?.issueKey||context?.extension?.issue?.key,assetId=payload?.assetId;if(!issueKey||!assetId)throw new Error('Issue and asset are required.');const asset=await kvs.get(`${ASSET_PREFIX}${assetId}`);if(!asset)throw new Error('Asset not found.');await kvs.set(`${LINK_PREFIX}${issueKey}`,{issueKey,assetId,deviceName:asset.name,linkedAt:now()});await addHistory(assetId,{type:'ticket-linked',source:'jira',message:`${issueKey} linked to asset`,issueKey});return{issueKey,asset};});
resolver.define('unlinkAssetFromIssue',async({payload,context})=>{const issueKey=payload?.issueKey||context?.extension?.issue?.key;if(!issueKey)throw new Error('Issue is required.');const link=await kvs.get(`${LINK_PREFIX}${issueKey}`);await kvs.delete(`${LINK_PREFIX}${issueKey}`);if(link?.assetId)await addHistory(link.assetId,{type:'ticket-unlinked',source:'jira',message:`${issueKey} unlinked from asset`,issueKey});return{ok:true};});
...
resolver.define('getAssetReport',async()=>{
  const assets=await queryAllByPrefix(ASSET_PREFIX);if(!assets.length)return[];const rows=[];let field=null,issues=[],settings=await getSettingsValue();try{const found=await searchIssuesWithConfiguredAssetField();field=found.field;issues=found.issues;settings=found.settings;}catch{}
  ...
  if(field){for(const issue of issues){const ticket=ticketFields(issue);for(const identifier of fieldValues(issue.fields?.[field.id]))addTicket(identifier,ticket,'primary');if(settings.jiraRelatedAssetField?.id)for(const identifier of relatedIdentifiers(issue.fields?.[settings.jiraRelatedAssetField.id]))addTicket(identifier,ticket,'related');}}
  for(const asset of assets){const key=normaliseName(asset.jiraIdentifier||asset.name);const tickets=[...(ticketsByIdentifier.get(key)?.values()||[])].sort((a,b)=>String(b.created||'').localeCompare(String(a.created||'')));const primary=tickets.filter(t=>t.relation==='primary');const related=tickets.filter(t=>t.relation==='related');const open=primary.filter(t=>!t.resolved&&t.statusCategory!=='done').length;rows.push({assetId:asset.id,name:asset.name,type:asset.type,status:asset.status,assigneeName:asset.assigneeName||asset.crewCode||'',total:primary.length,related:related.length,involved:tickets.length,open,resolved:primary.length-open,lastFault:primary.find(t=>t.created)?.created||'',latestFault:primary[0]||related[0]||null,error:false});}
