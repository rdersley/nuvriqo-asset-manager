...
function jiraAssetSearchShape(field,settings){const numericId=String(field.id).replace('customfield_','');const projectKey=clean(settings.jiraProjectKey||'').replace(/[^A-Za-z0-9_-]/g,'');const scope=projectKey?`project = \"${projectKey}\" AND `:'';return{jql:`${scope}cf[${numericId}] is not EMPTY ORDER BY created DESC`,fields:[field.id,settings.jiraRelatedAssetField?.id,settings.jiraClientField?.id,settings.jiraLocationField?.id,settings.jiraTypeField?.id,settings.jiraCrewCodeField?.id,settings.jiraFaultField?.id,'summary','status','issuetype','priority','assignee','created','resolutiondate','resolution'].filter(Boolean)};}
...
async function searchAssetTickets(assetId,legacyKeys=[]){
  ...
  if(validIdentifier(targetIdentifier)){
    ...
    if(field){for(const issue of issues){if(issueMatchesIdentifier(issue,field.id,targetIdentifier))matched.set(issue.key,ticketFields(issue,'primary',settings.jiraFaultField?.id));else if(issueMatchesRelatedIdentifier(issue,settings.jiraRelatedAssetField?.id,targetIdentifier))matched.set(issue.key,relatedTicketFields(issue,settings.jiraFaultField?.id));}}
  }
...
resolver.define('getAssetReport',async()=>{
  const assets=await queryAllByPrefix(ASSET_PREFIX,100);if(!assets.length)return[];const rows=[];const existingFaultHistory=await queryAllByPrefix(FAULT_HISTORY_PREFIX);const knownFaultHistoryKeys=new Set(existingFaultHistory.map(item=>item?.historyKey).filter(Boolean));let field=null,issues=[],settings=await getSettingsValue();try{const found=await searchIssuesWithConfiguredAssetField();field=found.field;issues=found.issues;settings=found.settings;}catch{}
