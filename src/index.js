...
async function refreshPortalPlusSnapshots(){try{const{field,issues}=await searchIssuesWithConfiguredAssetField();if(!field)return{ok:true,published:0,skipped:true,reason:'asset-field-not-configured'};const fields=await getJiraCustomFields();const organisationField=fields.find((f)=>normaliseName(f.name)==='organizations'||normaliseName(f.name)==='organisations'||normaliseName(f.customType).includes('organization'));if(!organisationField)return{ok:true,published:0,skipped:true,reason:'organisation-field-not-found'};const assets=await queryAllByPrefix(ASSET_PREFIX);return await publishPortalPlusSnapshotsForMappings({issues,assets,deviceFieldId:field.id,organisationFieldId:organisationField.id});}catch(error){console.warn('Portal+ asset snapshot refresh failed',error?.message||error);return{ok:false,published:0,error:String(error?.message||error)};}}
...
function jiraAssetSearchShape(field,settings){const numericId=String(field.id).replace('customfield_','');return{jql:`cf[${numericId}] is not EMPTY ORDER BY created DESC`,fields:[field.id,settings.jiraRelatedAssetField?.id,settings.jiraLocationField?.id,settings.jiraTypeField?.id,settings.jiraCrewCodeField?.id,'summary','status','issuetype','priority','assignee','created','resolutiondate','resolution'].filter(Boolean)};}
...
resolver.define('bulkImportAssets',async({payload})=>{
  ...
  await refreshPortalPlusSnapshots();return{imported,failed,deviceTypesAdded};
});
