...
function CrewTracking(){
 ...
 const filtered=useMemo(()=>report.filter(c=>{const q=query.trim().toLowerCase();const matches=!q||[c.crewCode,c.name,c.location,...(c.historicalDevices||[]),...(c.currentDeviceTypes||[]).map(g=>g.type)].some(v=>String(v||'').toLowerCase().includes(q));return matches&&(!onlyReview||c.reviewRequired);}),[report,query,onlyReview]);
