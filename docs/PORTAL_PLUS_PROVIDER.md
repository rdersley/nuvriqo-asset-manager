...
- **When:** hourly (`scheduledTrigger` `portal-plus-snapshot`), after a completed Jira scan, and on demand through the admin-only `publishPortalPlusSnapshot` resolver. The last result is stored under `portal-plus:last-publish` (`getPortalPlusStatus`).
- **Scope:** writing a project property needs `manage:jira-project`. It is currently declared in the internal `manifest.yml` only.
